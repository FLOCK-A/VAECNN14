#!/usr/bin/env python3
"""
Stage-1 City Teacher Training Script

Trains a city classification teacher on:
- Source domain (A) training data
- Target domain training data (half of target train split)
- Supervision: city labels only

Output: outputs/teacher_city/A2{target}/best.pth + last.pth
"""
import os
import sys
import json
...
def load_and_validate_data(json_path, target_name, source_domain=0):
    """
    Load dataset and validate against data leakage
    
    Args:
        json_path: Path to dataset JSON
        target_name: Target device name ('b', 'c', 's1', etc.)
    ...
    validate_no_leakage(source_samples + target_samples,
                       allowed_splits=['train'],
                       allowed_devices=[source_domain, target_domain],
                       phase='City Teacher training')
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    for batch in dataloader:
        ...
        labels = batch['city_label'].to(device)  # Use city labels
...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            features = batch['features'].to(device)
            labels = batch['city_label'].to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Train Stage-1 City Teacher')
    ...
    parser.add_argument('--seed', type=int, default=42,
                       help='Random seed')
    parser.add_argument('--output_dir', type=str, default='outputs/teacher_city',
    ...
    print("="*80)
    source_samples, target_samples = load_and_validate_data(
        args.json_path, args.target_name
    )
    ...
    # Create dataloaders (use city labels)
    train_loader = get_dataloader(
        ...
        label_key='city'  # Use city labels
    )
    ...
    # Create model
    print("\n" + "="*80)
    print("Creating City Teacher model...")
    print("="*80)
